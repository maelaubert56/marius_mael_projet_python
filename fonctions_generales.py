    ...
    with open('booksread.txt', 'r') as f_booksread:
        ...
        for ligne in f_booksread:
            ...
            for i in range(1, len(ligne)):
                ligne[i] = int(ligne[i])
                if ligne[i] in range(1, nblignes + 1) and (str(ligne[
                                                                   i]) not in liste_livres_lu):  # on ajoute la valeur seulement si la valeur est >0 et qu'elle n'est pas déja présente
...
def choisir_livre(liste_livres=list(range(1, nombreDeLignes("books.txt") + 1))):
    ...
    for i in range(
            len(livres_lus)):  # on transpose les valeurs récupéré aux emplacements réels des livres dans le fichier
